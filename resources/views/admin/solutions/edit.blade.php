...
<div class="max-w-4xl">
    <div class="bg-white rounded-lg shadow">
        <form method="POST" action="{{ route('admin.solutions.update', $solution) }}" id="solutionForm">
            @csrf
            @method('PUT')
            ...
            <div class="px-6 py-4 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="type" id="type" required
                            ...
                            <option value="video" {{ old('type', $solution->type) == 'video' ? 'selected' : '' }}>Video</option>
                            <option value="article" {{ old('type', $solution->type) == 'article' ? 'selected' : '' }}>Article</option>
                        </select>
                        @error('type')
                    ...
                    <div>
                        ...
                        <select name="pillar_tag" id="pillar_tag" required
                                class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                            <option value="heart" {{ old('pillar_tag', $solution->pillar_tag) == 'heart' ? 'selected' : '' }}>Heart</option>
                            <option value="grit" {{ old('pillar_tag', $solution->pillar_tag) == 'grit' ? 'selected' : '' }}>Grit</option>
                            <option value="wisdom" {{ old('pillar_tag', $solution->pillar_tag) == 'wisdom' ? 'selected' : '' }}>Wisdom</option>
                        </select>
                        @error('pillar_tag')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <input type="text" name="author_name" id="author_name"
                           value="{{ old('author_name', $solution->author_name) }}"
                ...
                <div>
                    ...
                    <select name="locale" id="locale" required
                        ...
                        @foreach($languages as $language)
                            <option value="{{ $language->code }}" {{ old('locale', $solution->locale) == $language->code ? 'selected' : '' }}>
                                {{ $language->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="border-t">
                <div class="px-6 py-4 border-b">
                    ...
                    <p class="text-sm text-gray-600">Edit content for each language. At least one translation is required.</p>
                </div>
                ...
                <div class="border-b">
                    <nav class="flex space-x-8 px-6" aria-label="Language tabs">
                        @foreach($languages as $index => $language)
                            @php
                                $translation = $solution->translations->where('locale', $language->code)->first();
                                $hasContent = $translation && !empty($translation->title) && !empty($translation->content);
                            @endphp
                            <button type="button" 
                                ...
                                {{ $language->name }}
                                <span class="ml-2 px-2 py-1 text-xs rounded-full {{ $hasContent ? 'bg-green-100 text-green-800' : 'bg-gray-100' }}">
                                    {{ strtoupper($language->code) }}
                                </span>
                                @if($translation && $translation->is_auto_generated)
                                    <i class="fas fa-robot text-blue-500 text-xs ml-1"></i>
                                @endif
                            </button>
                        @endforeach
                    </nav>
                            ...
                            <div class="space-y-6">
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">
                                        Title ({{ $language->name }})
                                        @if($language->code === old('locale', $solution->locale))
                                    ...
                                    <input type="text" 
                                            ...
                                            {{ $language->code === old('locale', $solution->locale) ? 'required' : '' }}
                                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                                ...
                                <div>
                                    ...
                                    <textarea name="translations[{{ $language->code }}][content]" 
                                              ...
                                              {{ $language->code === old('locale', $solution->locale) ? 'required' : '' }}
                                              class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">{{ old("translations.{$language->code}.content", $translation->content ?? '') }}</textarea>
                                    ...
                                    @if($translation && $translation->is_auto_generated)
                                        <p class="mt-1 text-sm text-blue-600">
                                            <i class="fas fa-robot"></i> This translation was auto-generated by AI
                                        </p>
                                    @endif
                                </div>
                            </div>
...
// Start with the primary language tab selected
document.addEventListener('DOMContentLoaded', function() {
    const primaryLocale = document.getElementById('locale').value;
    switchLanguage(primaryLocale);
});
</script>
@endsection
@endsection
