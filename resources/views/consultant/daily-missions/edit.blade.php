...
<div class="max-w-3xl mx-auto">
    <div class="mb-6 flex items-center justify-between">
        <h2 class="text-2xl font-bold text-white">Edit Daily Mission</h2>
    ...
    <div class="rounded-2xl bg-white/10 border border-white/15 backdrop-blur-xl p-6">
        <form method="POST" action="{{ route('consultant.daily-missions.update', ['locale' => app()->getLocale(), 'dailyMission' => $mission]) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                          class="mt-2 w-full rounded-xl bg-white/10 border border-white/15 text-white px-4 py-3 focus:outline-none focus:ring-2 focus:ring-white/30">{{ old('description', $mission->description) }}</textarea>
                @error('description')
                    <p class="mt-2 text-sm text-red-200">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-2">
                    ...
                    <label class="inline-flex items-center gap-2 text-sm text-white/80">
                        <input type="checkbox" name="is_body" value="1" class="rounded border-white/20 bg-transparent" @checked((bool) old('is_body', $mission->is_body))>
                        <span>Body</span>
                    </label>

                    <input type="hidden" name="is_mind" value="0">
                    <label class="inline-flex items-center gap-2 text-sm text-white/80">
                        <input type="checkbox" name="is_mind" value="1" class="rounded border-white/20 bg-transparent" @checked((bool) old('is_mind', $mission->is_mind))>
                        <span>Mind</span>
                    </label>

                    <input type="hidden" name="is_wisdom" value="0">
                    <label class="inline-flex items-center gap-2 text-sm text-white/80">
                        <input type="checkbox" name="is_wisdom" value="1" class="rounded border-white/20 bg-transparent" @checked((bool) old('is_wisdom', $mission->is_wisdom))>
                        <span>Wisdom</span>
                    </label>
                </div>
            </div>
