...
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        ...
        <h2 class="text-2xl font-bold text-white">Edit Mission Set</h2>
    </div>

    <div class="bg-white/10 backdrop-blur-xl border border-white/15 rounded-2xl p-6">
        <form method="POST" action="{{ route('consultant.mission-sets.update', ['locale' => app()->getLocale(), 'missionSet' => $missionSet]) }}">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $missionSet->getTranslation('name', app()->getLocale())) }}" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/50 placeholder-white/30" required>
                    @error('name')
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/50 placeholder-white/30">{{ old('description', $missionSet->getTranslation('description', app()->getLocale())) }}</textarea>
                    @error('description')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center justify-end gap-3 pt-4">
                    ...
                    <button type="submit" class="bg-emerald-500 hover:bg-emerald-400 text-white font-medium px-6 py-2 rounded-lg transition">Update Mission Set</button>
                </div>
            </div>
        </form>
    </div>
